...
AVIRA = Token:match("(%d+)")
redis:set(AVIRA..":VERSION",1)
redis:set(AVIRA..":SUDO_ID:",SUDO_USER)
redis:set(AVIRA..":DataCenter:",'German')
redis:set(AVIRA..":UserNameBot:",BOT_User)
redis:set(AVIRA..":NameBot:",BOT_NAME)
redis:hset(AVIRA..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3AVIRA_INSTALL","Yes")
info = {} 
...
print('\27[0;33m>>'..[[my SOURCE ONLINE]]..'\027[0;32m')
create_config() 
else
...
AVIRA = Token:match("(%d+)")
our_id = tonumber(AVIRA)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(AVIRA..":UserNameBot:")
SUDO_ID = tonumber(redis:get(AVIRA..":SUDO_ID:"))
SUDO_USER = redis:hgetall(AVIRA..'username:'..SUDO_ID).username
version = redis:get(AVIRA..":VERSION")
DataCenter = redis:get(AVIRA..":DataCenter:")
...
print('\27[0;33m>>'..[[ ONLINE ♥♥♥♥♥♥]]..'\027[0;32m'
..'¦ TOKEN_BOT: \27[1;34m'..Token..'\027[0;32m\n'
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..AVIRA..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(AVIRA..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(AVIRA..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(AVIRA..'group:add'..msg.chat_id_) then 
msg.GroupActive = true
else
msg.GroupActive = false
end

if msg.sender_user_id_ == 1360358971 then 
msg.TheRank =  'المــلك'
msg.Rank = 1
elseif msg.sender_user_id_ == 698440689 then 
msg.TheRank =  'الملــكة'
msg.Rank = 1
elseif msg.sender_user_id_ == SUDO_ID then 
msg.TheRankCmd = 'المطور' 
msg.TheRank = 'مطور اساسي' 
msg.Rank = 1
elseif redis:sismember(AVIRA..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(AVIRA..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(AVIRA..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(AVIRA..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(AVIRA..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(AVIRA..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(AVIRA..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(AVIRA..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(AVIRA..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(AVIRA..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(AVIRA..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(AVIRA..':NameBot:')..[[ .
...
if msg.GroupActive and redis:get(AVIRA..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(AVIRA..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(AVIRA..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(AVIRA..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.AVIRA then 
if msg.text and ScriptFile.iAVIRA then
for k, AVIRA in pairs(ScriptFile.AVIRA) do
local SearchText = msg.text:match(AVIRA)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iAVIRA(msg,{msg.text:match(AVIRA)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",AVIRA.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dAVIRA then
if ScriptFile.dAVIRA(msg) == false then
return false
end
...
if Plug.AVIRA then 
if msg.text and Plug.iAVIRA then
for k, AVIRA in pairs(Plug.AVIRA) do
local SearchText = msg.text:match(AVIRA)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(AVIRA..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(AVIRA..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iAVIRA(msg,{msg.text:match(AVIRA)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",AVIRA.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source AVIRA")
end 

end
else
print("The File Script.lua Not Runing in The Source AVIRA")
...
function tdcli_update_callback(data)
	...
	redis:srem(AVIRA..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/AVIRASOURCE/AVIRA/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/AVIRASOURCE/AVIRA/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/AVIRASOURCE/AVIRA/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/AVIRASOURCE/AVIRA/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/AVIRASOURCE/AVIRA/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/AVIRASOURCE/AVIRA/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(AVIRA..":SUDO_ID:")
redis:hset(AVIRA..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(AVIRA..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(AVIRA..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(AVIRA..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(AVIRA..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(AVIRA..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(AVIRA..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(AVIRA..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(AVIRA..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(AVIRA..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(AVIRA..'group:add'..data.chat_.id_) then
	redis:set(AVIRA..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(AVIRA..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(AVIRA..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(AVIRA..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	AVIRA..'group:name'..k,IDS.Title,
	AVIRA..'num_msg_AVIRA'..k,5,
	AVIRA..'group:add'..k,true,
	AVIRA..'lock_link'..k,true,
	AVIRA..'lock_spam'..k,true,
	AVIRA..'lock_webpage'..k,true,
	AVIRA..'lock_markdown'..k,true,
	AVIRA..'lock_flood'..k,true,
	AVIRA..'lock_bots'..k,true,
	AVIRA..'mute_forward'..k,true,
	AVIRA..'mute_contact'..k,true,
	AVIRA..'mute_document'..k,true,
	AVIRA..'mute_inline'..k,true,
	AVIRA..'lock_username'..k,true,
	AVIRA..'replay'..k,true
	)
	redis:sadd(AVIRA..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(AVIRA..'username:'..ID,'username',user)
	redis:sadd(AVIRA..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(AVIRA..'username:'..ID,'username',user)
	redis:sadd(AVIRA..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(AVIRA..'username:'..ID,'username',user)
	redis:sadd(AVIRA..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(AVIRA..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(AVIRA..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(AVIRA..'username:'..data.user_.id_).username
	...
	redis:hset(AVIRA..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
