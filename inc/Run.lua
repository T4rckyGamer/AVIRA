...
local url =  https://api.telegram.org/bot ..config.bot_api_key.. /getChatMember?chat_id=-ايديقناتك&user_id= ..msg.from.id..  
local res = HTTPS.request(url)
local jres = JSON.decode(res)	
if not jres.result then  api.sendMessage(msg.chat.id,  لايمكن استخدام البوت الى بعد الاشتراك في القناة @RTTRN , true)	return false end	 
if jres.result.status ==  left  then api.sendMessage(msg.chat.id,  يرجى الاشتراك في القناة لاستخدام البوت @RTTRN , true)	return false end	
	keyboard = {} 
	keyboard.inline_keyboard = {				
			{
		{text = "قناة البوت", url="https://t.me/RTTRN"},					
			},			
	}
local start = client:get( start )
api.sendKeyboard(msg.chat.id, (start or "no"), keyboard, true)       
return false end	    
